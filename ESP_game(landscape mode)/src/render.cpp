...
void drawSelectScreen(uint8_t wpnSel, uint8_t skillSel,
  ...
  // 标题
  Hardware::sprDraw->drawString("SELECT WEAPON & SKILL", SCR_W / 2, 10, 2);

  Hardware::sprDraw->setTextColor(TFT_WHITE, TFT_BLACK);
  Hardware::sprDraw->drawString("WEAPON (L/R)", SCR_W / 2, 24, 1);
  ...
  for (uint8_t i = 0; i < WPN_COUNT; i++) {
    ...
    Hardware::sprDraw->drawString(wpnDesc[i], SCR_W / 2, y + 9, 1);
  }
  ...
  Hardware::sprDraw->setTextColor(TFT_WHITE, TFT_BLACK);
  Hardware::sprDraw->drawString("SKILL (U/D)", SCR_W / 2, 120, 1);
  ...
  for (uint8_t i = 0; i < SKILL_COUNT; i++) {
    ...
    Hardware::sprDraw->setTextColor(TFT_DARKGREY, TFT_BLACK);
    Hardware::sprDraw->drawString(sklDesc[i], SCR_W / 2, y + 9, 1);
  }
...
void drawCalibrate(uint16_t rawX, uint16_t rawY, uint8_t step,
  ...
  Hardware::sprDraw->drawString("JOYSTICK CAL", SCR_W / 2, 6, 2);
  ...
  if (step < 2) {
    ...
    Hardware::sprDraw->drawString(buf, SCR_W / 2, 20, 1);

    Hardware::sprDraw->setTextColor(TFT_WHITE, TFT_BLACK);
    if (step == 0) {
      Hardware::sprDraw->drawString("Push UP ^ then press B", SCR_W / 2, 32, 1);
    } else {
      Hardware::sprDraw->drawString("Push RIGHT > then press B", SCR_W / 2, 32, 1);
    }
  } else {
    Hardware::sprDraw->setTextColor(TFT_GREEN, TFT_BLACK);
    Hardware::sprDraw->drawString("CALIBRATION DONE", SCR_W / 2, 20, 1);

    Hardware::sprDraw->setTextColor(TFT_YELLOW, TFT_BLACK);
    snprintf(buf, 32, "SwapXY:%s Xinv:%s Yinv:%s",
             swapXY ? "YES" : "NO", invX ? "YES" : "NO", invY ? "YES" : "NO");
    Hardware::sprDraw->drawString(buf, SCR_W / 2, 32, 1);
  }
  ...
  // 可视化：摇杆位置（缩小为 120x120，适配横屏高度）
  int16_t boxX = 10, boxY = 60, boxW = 120, boxH = 120;
  int16_t dotX = boxX + (int16_t)rawX * (boxW - 1) / JOY_MAX;
  int16_t dotY = boxY + (int16_t)rawY * (boxH - 1) / JOY_MAX;
  Hardware::sprDraw->drawRect(boxX, boxY, boxW, boxH, TFT_WHITE);
  Hardware::sprDraw->fillCircle(dotX, dotY, 3, TFT_RED);

  // 四个角标
  Hardware::sprDraw->drawRect(boxX, boxY, 6, 6, TFT_CYAN);                      // 左上 (0,0)
  Hardware::sprDraw->drawRect(boxX + boxW - 6, boxY, 6, 6, TFT_CYAN);           // 右上 (max,0)
  Hardware::sprDraw->drawRect(boxX, boxY + boxH - 6, 6, 6, TFT_CYAN);           // 左下 (0,max)
  Hardware::sprDraw->drawRect(boxX + boxW - 6, boxY + boxH - 6, 6, 6, TFT_CYAN); // 右下 (max,max)

  // 方向高亮指示
  if (step == 0) {
    Hardware::sprDraw->drawRect(boxX + boxW / 2 - 8, boxY - 14, 16, 16, TFT_YELLOW);
    Hardware::sprDraw->setTextColor(TFT_YELLOW, TFT_BLACK);
    Hardware::sprDraw->drawString("UP", SCR_W / 2, boxY - 28, 1);
  } else if (step == 1) {
    Hardware::sprDraw->drawRect(boxX + boxW + 6, boxY + boxH / 2 - 8, 16, 16, TFT_YELLOW);
    Hardware::sprDraw->setTextColor(TFT_YELLOW, TFT_BLACK);
    Hardware::sprDraw->drawString("RIGHT", boxX + boxW + 30, boxY + boxH / 2 - 4, 1);
  }

  // 底部操作提示
  Hardware::sprDraw->setTextColor(TFT_WHITE, TFT_BLACK);
  Hardware::sprDraw->drawString("Press A to return", SCR_W / 2, 225, 1);
